...
function EditUser({ user, showModal, userUpdated, closeModal }) {
  ...
  const editUser = (e) => {
    ...
    Axios.put(`http://localhost:3001/editUser/${user._id}`, { firstname, lastname, email, actions })
      .then((response) => {
                        ...
                        <div className="relative w-[448px] h-[366px] bg-white rounded-md shadow flex-col justify-start items-center inline-flex">
                            ...
                            <div className="self-stretch h-[234px] px-6 py-2 flex-col justify-start items-start gap-2.5 flex">
                                <div className="self-stretch justify-start items-start inline-flex">
                                    ...
                                    <input type="text" className="grow shrink basis-0 h-6 px-2 bg-white rounded-tr-sm rounded-br-sm border border-slate-200 flex-col justify-center items-center gap-2.5 inline-flex font-normal text-xs" placeholder='Jon' value={firstname} onChange={(e) => setFirstname(e.target.value)} />
                                </div>
                                <div className="self-stretch justify-start items-start inline-flex">
                                    <div className="w-[72px] h-6 px-2 bg-slate-100 rounded-tl-sm rounded-bl-sm border border-slate-200 justify-center items-center gap-2.5 flex">
                                        <div className="text-black text-xs font-normal ">Lastname</div>
                                    </div>
                                    <input type="text" className="grow shrink basis-0 h-6 px-2 bg-white rounded-tr-sm rounded-br-sm border border-slate-200 flex-col justify-center items-center gap-2.5 inline-flex font-normal text-xs" placeholder='Doe' value={lastname} onChange={(e) => setLastname(e.target.value)}/>
                                </div>
                                <div className="self-stretch justify-start items-start inline-flex">
                                    <div className="w-[72px] h-6 px-2 bg-slate-100 rounded-tl-sm rounded-bl-sm border border-slate-200 justify-center items-center gap-2.5 flex">
                                        <div className="text-black text-xs font-normal ">Email</div>
                                    </div>
                                    <input type="email" className="grow shrink basis-0 h-6 px-2 bg-white rounded-tr-sm rounded-br-sm border border-slate-200 flex-col justify-center items-center gap-2.5 inline-flex font-normal text-xs" placeholder='j.doe@email.com' value={email} onChange={(e) => setEmail(e.target.value)}/>
                                </div>
                            ...
                            <div className="self-stretch h-[72px] px-6 py-4 flex-col justify-start items-end gap-2.5 flex">
                                <div className="self-stretch justify-end items-center gap-3 inline-flex">
                                    <button type="button" className="w-[86px] h-10 px-4 bg-slate-100 rounded-md justify-center items-center gap-2 flex text-gray-900 text-base font-semibold leading-normal" onClick={closeModal}>Cancel</button>
                                    <button type="submit" className="w-[85px] h-10 px-4 bg-blue-500 rounded-md justify-center items-center gap-2 flex text-white text-base font-semibold leading-normal" onClick={() => editUser()}>Save</button>
                                </div>
                            </div>
                        </div>                      
                    </form>
